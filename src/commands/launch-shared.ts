...
export async function launchClaudeForProfile(
  ...
  const binding = options.fresh
    ? null
    : await context.runtime.sessionStore.get(projectKey(context.process.cwd()), profile.id);
  ...
  outro(
    binding && !options.fresh
      ? `Claude exited. Resume source: ${binding.sessionId}`
      : "Claude exited.",
  );
}
